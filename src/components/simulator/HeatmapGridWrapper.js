...
import HeatmapGrid2 from './HeatmapGrid2'
...
export default function HeatmapGridWrapper({ HMResults, viewValues, viewHover }) {
  ...
  return (
    <Center w={`${totalW}px`} h={`${totalH}px`} cn='rel bsh3 bc2-3' >
      {
        HMResults.data?.length > 0 ? <HeatmapGrid2 HMResults={HMResults} gridW={gridW} gridH={gridH} tickLen={tickLen} axisTitleLen={axisTitleLen} viewValues={viewValues} viewHover={viewHover} /> : null
      }
    </Center>
  )
}
